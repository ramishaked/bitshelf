...
- **packages/i18n:** i18next עם he.json ו-en.json, עברית ברירת מחדל. RTL נאכף ב-`I18nManager` בהפעלה הראשונה של האפליקציה (flip ו-reload חד-פעמי).
...
- bundle של iOS מתקמפל נקי דרך `expo export --platform ios`, כולל כל ה-imports מהחבילות המשותפות.
- הרצה בסימולטור עדיין לא בוצעה, ראה סעיף הבא.

## מה צריך להגדיר ידנית

1. **Xcode:** לא מותקן על המכונה (יש רק Command Line Tools), ולכן אי אפשר להריץ סימולטור iOS. להתקין Xcode מה-App Store, לפתוח פעם אחת לאישור הרישיון, ואז:
   ```
   sudo xcode-select -s /Applications/Xcode.app/Contents/Developer
   ```
   אחרי זה `pnpm --filter @bitshelf/mobile ios` יפתח את האפליקציה ב-Expo Go בסימולטור.
2. **Clerk:** ליצור אפליקציה ב-dashboard, להפעיל Apple, Google ו-email code, ולמלא CLERK_SECRET_KEY, NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY ו-EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY. בלי מפתחות, המובייל רץ במצב פיתוח בלי התחברות (ישר לטאבים) וה-web מדלג על ה-middleware.
3. **Neon:** ליצור פרויקט ולמלא DATABASE_URL, ואז להריץ push ו-seed (פקודות למעלה).
4. **R2:** ליצור bucket ו-API token עם הרשאת קריאה וכתיבה לאובייקטים, ולמלא את חמשת משתני R2_*.

## הערות

- הריפו יושב בתיקיית Google Drive מסונכרנת. node_modules מכיל עשרות אלפי קבצים, כדאי לסמן אותו כ"לא לסנכרן" בהגדרות Drive או להזיז את הריפו החוצה.
