...
export const modelReferenceSourceEnum = pgEnum("model_reference_source", [
  "ai_generated",
  "user_edited",
  "curated",
]);
...
// One record per model, shared across users (spec 4.6.1). Generated by AI
// with web search after an item is saved; the catalog never waits for it.
export const modelReferences = pgTable(
  "model_references",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    collectionTypeId: uuid("collection_type_id")
    ...
    // empty string when the model has no variant, keeps the unique key simple
    variant: text("variant").notNull().default(""),
    releaseYear: integer("release_year"),
    discontinuedYear: integer("discontinued_year"),
    summary: jsonb("summary").$type<LocalizedText>().notNull(),
    // CPU, speed, RAM, graphics, sound, media, ports, launch price
    specs: jsonb("specs").$type<Record<string, string>>().notNull().default({}),
    // [{ label, url }], every URL verified before save (spec 4.6.1)
    links: jsonb("links").$type<{ label: string; url: string }[]>().notNull().default([]),
    // "worth knowing" bullets per the approved design (artboard 05)
    tips: jsonb("tips").$type<{ he?: string[]; en?: string[] }>().notNull().default({}),
    knownVersions: text("known_versions"),
    source: modelReferenceSourceEnum("source").notNull().default("ai_generated"),
    generatedAt: timestamp("generated_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
    uniqueIndex("model_references_key_idx").on(t.manufacturer, t.model, t.variant),
  ],
);
